...
/**
 * This class used for showing list of added menu in cart.
...
public class FragmentCart extends Fragment implements AdapterCartList.myInterface, APIRequest.ResponseHandler {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        /*********************************************************************************/
        etCustomeVal.setText(Config.tip);
    ...
    private void inItWidgets() {
        ...
        etCustomerComment.addTextChangedListener(new TextWatcher() {
            @Override
            ...
            public void afterTextChanged(Editable s) {
                System.out.println("***********comment change********");
                String customerComment = etCustomerComment.getText().toString().trim();
                if (!customerComment.isEmpty()) {
                    editor.putString("cartComment", customerComment);
                    editor.commit();
  ...
  //click on no tip
        ...
        lltwentyper.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                isTipCustomlicked = false;
                Config.ClearSalesAmnt = true;
                System.out.println("********2000000********" + Config.ClearSalesAmnt);
       ...
       /* etCustomeVal.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
        ...
        etCustomeVal.setOnFocusChangeListener(new View.OnFocusChangeListener() {
            ...
            public void onFocusChange(View v, boolean hasFocus) {
                ...
                Config.tipStatus = "custom";
                isTipNoneClicked = false;
                isTip15Clicked = false;
                isTip20Clicked = false;
                isTipCustomlicked = true;
                System.out.println("***sales amnttttttt*****" + Config.ClearSalesAmnt);
                if (hasFocus) {
                    System.out.println("*focus*******"+Config.customTipAmnt);
                    if(Config.customTipAmnt.equalsIgnoreCase("0.00") || Config.customTipAmnt.equalsIgnoreCase("")) {
                        Config.customTipAmnt = "0.00";
                    }
                    tvTipAmount.setText("$" + Config.customTipAmnt);
                }
                if (Config.ClearSalesAmnt) {
                    System.out.println("******etCustomeVal*****if");

                    Config.ClearSalesAmnt = false;
                }
                llCustome.setBackground(getResources().getDrawable(R.drawable.edt_green_border));
                lltwentyper.setBackground(getResources().getDrawable(R.drawable.edt_white_cellborder));
                llNoTip.setBackground(getResources().getDrawable(R.drawable.edt_white_cellborder));
        ...
        etCustomeVal.addTextChangedListener(new TextWatcher() {
                    ...
                    if (!resStr.isEmpty()) {
                        if (!resStr.equals(".") && !resStr.equals("")) {
                            ...
                            if (lengthWithDots - lengthWithoutDots >= 2) {
                                ...
                                System.out.println("*********else*********"+tvTipAmount.getText().toString());
            ...
            public void afterTextChanged(Editable s) {
                ...
                Config.customTipAmnt = etCustomeVal.getText().toString();
                Config.tip = etCustomeVal.getText().toString();
            }
        });
    ...
    private void normalValInit() {
        ...
        System.out.println("*******tipStatus******"+Config.tipStatus);
        if(Config.tipStatus.equalsIgnoreCase("no tip"))
        {
            //hideSoftKeyboard(((UDashboardActivityNew) getActivity()), v); // MainActivity is the name of the class and v is the View parameter used in the button listener method onClick.
            Config.customTipAmnt = "";
            isTipNoneClicked = true;
            ...
            String finalTax3 = getFinalTaxValue(tvOrderSubTotal.getText().toString().replace("$", "").trim(), RESTAURANT_SALES_TAX);
            String strFinalTax = getFinalFormattedVal(finalTax3);
            tvSalesTax.setText("$" + String.valueOf(strFinalTax));
        ...
        }else if(Config.tipStatus.equalsIgnoreCase("15%"))
        {
            Config.ClearSalesAmnt = true;
            Config.customTipAmnt = "";
            //hideSoftKeyboard(((UDashboardActivityNew) getActivity()), v);
            ...
            tvTipAmount.setText("$" + tvFifteenPercentage.getText().toString().trim().replace("$", "").trim());
            String finalTax2 = getFinalTaxValue(tvOrderSubTotal.getText().toString().replace("$", "").trim(), RESTAURANT_SALES_TAX);
            String strFinalTax = getFinalFormattedVal(finalTax2);
            tvSalesTax.setText("$" + String.valueOf(strFinalTax));
            String mSalesTax = "";
            ...
            System.out.println(">> tip " + tip);
            System.out.println(">> finalPlaceOrderAmt " + finalPlaceOrderAmt);
        }else if(Config.tipStatus.equalsIgnoreCase("20%"))
        {
           // InputMethodManager imm = (InputMethodManager) getActivity().getSystemService(Context.INPUT_METHOD_SERVICE);
            //imm.hideSoftInputFromWindow(view.getApplicationWindowToken(), 0);
            ...
            String finalTax1 = getFinalTaxValue(tvOrderSubTotal.getText().toString().replace("$", "").trim(), RESTAURANT_SALES_TAX);

            String strFinalTax = getFinalFormattedVal(finalTax1);
            tvSalesTax.setText("$" + String.valueOf(strFinalTax));
        ...
        }else if(Config.tipStatus.equalsIgnoreCase("custom"))
        {
            isTipNoneClicked = false;
            isTip15Clicked = false;
            isTip20Clicked = false;
